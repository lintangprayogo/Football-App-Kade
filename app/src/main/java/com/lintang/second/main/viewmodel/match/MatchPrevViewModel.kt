...
class MatchPrevViewModel(val context: Application, private val repository: Repository) :
    BaseViewModel(context) {
    var matchPrevListLive = SingleLiveEvent<List<SingleMatch>>()


    fun getPrevMatch(idLeague: String) {
        repository.getPrevMatch(idLeague, object : DataSource.GetRemoteCallback<List<SingleMatch>> {
            ...
            override fun onSuccess(data: List<SingleMatch>) {
                if (data.isEmpty()) {
                    ...
                    matchPrevListLive.postValue(data)
                }
            }
